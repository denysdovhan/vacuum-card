...
- **Most Home Assistant custom elements (`ha-select`, `ha-textfield`, `ha-switch`, `mwc-list-item`, `ha-card`, `ha-icon`, `ha-dropdown`, `ha-spinner`) are undefined outside HA's real frontend.** They render as inert unknown elements — properties like `.value`/`.configValue` set via Lit bindings still work as plain JS properties, but the components' own behavior (e.g. `ha-textfield` dispatching `input` when a user types) doesn't exist. The harness works around this by setting `.value` directly on the target element and dispatching the same `input`/`config-changed` events the real widgets would, which exercises the actual `editor.ts` `valueChanged()` handler rather than mocking it away. **This also means the harness cannot catch an element being silently absent from HA's real frontend bundle** — it happened once already (`paper-input`, a legacy Polymer element HA has been dropping, rendered fine here but was invisible in a real recent HA version). If a field mysteriously doesn't appear on a real install despite passing here, suspect the tag itself before suspecting the binding logic.
