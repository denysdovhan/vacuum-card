...
- **Most Home Assistant custom elements (`ha-select`, `ha-switch`, `mwc-list-item`, `ha-card`, `ha-icon`, `ha-dropdown`, `ha-spinner`) are undefined outside HA's real frontend.** They render as inert unknown elements — properties like `.value`/`.configValue` set via Lit bindings still work as plain JS properties, but the components' own behavior doesn't exist. The harness works around this by setting `.value` directly on the target element and dispatching the same event the real widget would (e.g. `input`, `config-changed`), which exercises the actual `editor.ts` `valueChanged()` handler rather than mocking it away.
- **This harness cannot catch an editor field being silently invisible on a real HA install — it happened twice.** First `paper-input` (a legacy Polymer element HA has dropped from its bundle) rendered fine here but was an inert, invisible zero-height element on a real recent HA version. After swapping to `ha-textfield`, the _same symptom recurred_ — HA's newer versions use a **Scoped Custom Element Registry** to isolate its own internal Material components from third-party custom cards, so an `ha-*` element can exist in HA's own UI while being genuinely unregistered (no shadow root, never upgraded) in a third-party card's scope. Confirmed on a real install via DevTools Elements panel: the `<ha-textfield>` nodes existed in the DOM but had no `#shadow-root` and no expand arrow. Editor text inputs in this project now use a plain native `<input>` for exactly this reason — it has no dependency on any custom element registry, scoped or global, so this entire class of bug is structurally impossible for it. Don't reach for `ha-textfield`/`paper-input`/similar `ha-*`-prefixed form controls in this editor without confirming they're globally exposed on a real, current HA install first — this harness cannot verify that for you.
